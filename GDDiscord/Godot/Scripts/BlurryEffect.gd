...
func blurry_start(transition_s : float = 0.5, blurry_max : float = 3.0):
	blurry_value = 0.01
	blurry_node.show()
	print_debug("blurry_value START: ", blurry_value)
...
func blurry_stop(transition_s : float = 0.5, blurry_max : float = 3.0):
	...
	print_debug("blurry_value STOP: ", blurry_value)
	...
	get_tree().create_timer(transition_s).timeout.connect(blurry_node.hide)
